...
function onCreatePost()
    ...
    -- setProperty('timeBar.scale.x', 2.0);
    -- setProperty('timeBar.scale.y', 1.0);
    -- setProperty('timeBarBG.scale.x', 2.0);
    -- setProperty('timeBarBG.scale.y', 1.0);
...
function onCreate()
    ...
    setTextFont('msTxt', 'NiseSegaSonic.ttf')
    addLuaText('msTxt')

    makeLuaText('funnikade', songName .. ' ' .. difficultyName .. ' - PE ' .. version,getProperty('healthBarBG.width'), 0 ,680)
    setTextSize('funnikade', 16)

    makeLuaText('oldScore', '', 630, 300)
	setTextSize('oldScore', 16)

    -- so positions won't be fucked up -Daninnocent
...
function onUpdatePost(elapsed)
    ...
    if curSongPosRightNow - lastMsShowUp > msDiffTimeLimit and msTextVisible then
        ...
        setTextString('msTxt', 'NiseSegaSonic.ttf')
        addLuaText('msTxt')
        msTextVisible = false
    end
    ...
    if ratingName == '?' then
        local beforeScoreTxt = 'Score:0 | Combo Breaks:0 | Accuracy:0% | N/A'
        ...
        local finalScoreTxt = 'Score:'..score..' | Combo Breaks:'..misses..' | Accuracy:'..ratingFullAsStr..'% | ('..ratingNameM..') '..tempRatingNameVery
